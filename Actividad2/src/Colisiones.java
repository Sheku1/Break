...
public class Colisiones extends Applet implements Runnable, KeyListener {
    ...
    public void init() {
        ...
        URL eaURL = this.getClass().getResource("musica/elephant.wav");
        sonido = getAudioClip(eaURL);
        URL raURL = this.getClass().getResource("musica/mice.wav");
        ...
        while (inco != 0) {
            int posrX = (int) (Math.random() * (getWidth() / 4)) + getWidth() / 4;    //posision x es tres cuartos del applet
            int posrY = (int) (Math.random() * (getHeight() / 4)) + getHeight() / 4;    //posision y es tres cuartos del applet
            URL rURL = this.getClass().getResource("Imagenes/mouse.gif");
    ...
    public void paint(Graphics g) {
        ...
        if (chioco) {
            g.drawString(" Has Chocado = " + contador + " veces", 500, 200);
        }
